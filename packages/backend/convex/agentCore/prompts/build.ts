...
const toolContextSection = () =>
  ...
  `- find_friend: look up one of the user's friends by name; returns candidates with userId and a canViewCalendar flag. Use this ONLY when the user names a specific person to schedule with.\n` +
  `- get_friend_schedule: read a friend's calendar in a time window. Requires a userId from find_friend. Returns FAILED if the friend has disabled sharing — in that case, finish with status error explaining that sharing is off for that friend.\n` +
...
const friendSchedulingSection = () =>
  ...
  `2. If the candidate's canViewCalendar is false, STOP: call finish_agent with status error explaining that the friend has turned off calendar sharing with this user. Do not call get_friend_schedule, do not propose a time.\n` +
  `3. Otherwise, call get_user_schedule AND get_friend_schedule for the same candidate window in parallel. Find the earliest slot that satisfies step-2 timing constraints, preserves ~10-15 min padding vs adjacent events on BOTH calendars, and does not overlap anything on either calendar.\n` +
  `4. Treat the friend's events with the same busy-by-default rule as the user's — but some events ARE reasonably skippable: low-stakes solo blocks ("read demis", "ML study", "deep work", "focus time", "reading", generic untitled blocks), tentative/held time. NOT skippable: anything with another person's name, classes, meetings, appointments, travel, exercise the user clearly cares about, anything labeled with a specific deliverable. If the only way to fit the event is to overlap a friend's skippable solo block, that's acceptable — but prefer shifting the time first. Never overlap a hard block on either side.\n` +
  `5. Emit ONE propose_event_creation proposal on the user's calendar. Do not also propose on the friend's calendar — the user is only scheduling their own side for now. Put the friend's name in the summary ("Lunch with Jude"). Do not set attendees, description, or location unless the user specified them.`;
