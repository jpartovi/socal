...
const toolContextSection = () =>
  ...
  `Optional participantFriendUserIds: pass Convex user ids from find_friend so those friends receive Google Calendar invites when the user accepts. When set, every option must also fit each friend's calendar — validation applies to the user's AND each participant's calendar. ` +
  ...
  `If a call returns text starting with "FAILED — propose_event_creation", NO proposals were created — validation is all-or-nothing. Read which option failed and why (bad ISO, overlap, <15 min gap, time-of-day window, friend busy), then fix or drop that option and retry the whole batch. Do not repeat the same args. ` +
...
const schedulingProcessSection = () =>
  ...
  `3. Where does it fit in the schedule? First pass: call get_free_slots with friendUserIds [] (solo), with slotDurationMinutes and paddingMinutes matching the event and your buffer needs. The search must be forward-looking only: [startIso, endIso) should be something like [now, EOD) or [now, EOW) using the "Right now" time from the time context (never start at midnight for "the rest of today" when part of the day is already over). For a looser ask, use a wide forward range through the full candidate window from step 2, not a sliver of an hour. A returned slot is a time range with literally no timed calendar event in that range for the user, after the tool's padding; all-day entries do not count as busy for this tool. If get_free_slots finds no acceptable slot, call get_user_schedule ONCE covering the FULL candidate window from step 2 (plus a little padding) and pick a time using event titles: treat blocks as skippable only when the guidelines allow. If get_free_slots did find slots, you may still call get_user_schedule for a final sanity check before proposing. Within the window from step 2, pick slots such that (a) they do not overlap any existing event, and (b) there is at least ~10 min of gap between the new event and the adjacent event on either side. Prefer earlier days over later days only as a weak tiebreaker — a clean slot on Thursday beats a cramped one today. Never finish with "no time available" until you have surveyed the entire multi-day window; today being blocked is not an answer.\n` +
...
const friendSchedulingSection = () =>
  ...
  `2. First pass: call get_free_slots with that friend's userId (or several ids if multiple people are in the request — resolve each with find_friend), slotDurationMinutes matching the planned event length, and paddingMinutes for buffer (defaults are fine when unsure). Use a WIDE and **future-only** [startIso, endIso) covering the multi-day candidate window (today + the next ~6 days unless the user named a day). Never anchor at midnight for a same-day request when the current time is later in the day. More people means a longer forward net. A returned free window means: under the tool's rules, there is no timed calendar event in that range for the user or each friend, after padding — i.e. everyone is clear of real timed blocks. This ignores all-day; it does not judge skippable vs hard events (anything timed counts as busy). If get_free_slots yields good candidates, you can still glance at get_user_schedule + get_friend_schedule before proposing to double-check. If get_free_slots returns no useful slot, call get_user_schedule AND get_friend_schedule in parallel on forward windows and read event titles: find slots that respect padding where overlap is only against skippable blocks (per below), or shift per the general scheduling process. Within the window, find 2–3 distinct mutually-free slots that satisfy the time-of-day constraint for the activity ("dinner" means 6pm–9pm, never 3pm), preserve ~10–15 min padding vs adjacent events on BOTH calendars, and don't overlap anything hard on either calendar. One option only when the user pinned a specific time; otherwise always propose multiple.\n` +
  ...
  `4. Emit ONE propose_event_creation call on the user's calendar (the options-array version — one call, 1–3 options). Do not also propose on the friend's calendar — the user is only scheduling their own side for now. Follow the event proposal guidelines: neutral activity summary (e.g. "Lunch"), and pass that friend's userId in participantFriendUserIds so they get a Google Calendar invite on accept. When offering multiple options for a friend hang-out, EVERY option must be free on BOTH calendars — don't include an option where the friend is busy just to reach three.`;
